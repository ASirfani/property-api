...
// POST route to check if the decoded token values match with the server
router.post('/admin/check', async (req, res) => {
    const { id:_id, name } = req.body;
  
    try {
      // Retrieve the admin with the provided ID
      const admin = await Admin.findById(_id);
      if (!admin) {
        return res.json({ success: false });
      }
  
      // Compare the name from the decoded token with the admin's username
