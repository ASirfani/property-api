...
router.post('/suppliers/login', async (req, res) => {
  ...
  try {
    ...
    if (supplier && password === supplier.password) {
      const payload = { id: supplier._id, name: supplier.name,};
...
router.post('/supplier/check', async (req, res) => {
  ...
  try {
    // Retrieve the supplier with the provided ID
    const supplier = await Supplier.findById(_id);
    if (!supplier) {
      return res.json({ success: false });
    }

    // Compare the name from the decoded token with the supplier's email
